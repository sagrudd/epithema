...
pub(crate) fn infer_molecule_kind(residues: &str) -> MoleculeKind {
    ...
    let dna_like = epithema_core::Alphabet::Dna
        .validate(MoleculeKind::Dna, &uppercase)
        .is_ok();
    let rna_like = epithema_core::Alphabet::Rna
        .validate(MoleculeKind::Rna, &uppercase)
        .is_ok();
    if !dna_like
        && !rna_like
        && epithema_core::Alphabet::Protein
...
#[cfg(test)]
mod tests {
    use epithema_core::{Interval, Strand};
