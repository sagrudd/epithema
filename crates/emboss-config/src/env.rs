...
impl ConfigEnvironment {
        ...
        V: AsRef<str>,
    {
        for (key, value) in overrides {
            match key.as_ref() {
                "EMBOSS_RS_DATA_DIR" => config.paths.data_root = value.as_ref().into(),
                "EMBOSS_RS_CACHE_DIR" => config.paths.cache_root = value.as_ref().into(),
                "EMBOSS_RS_DOCS_ASSET_DIR" => config.paths.docs_asset_root = value.as_ref().into(),
                "EMBOSS_RS_ALLOW_REMOTE" => {
                    if let Some(value) = parse_bool(value.as_ref()) {
                        config.acquisition.allow_remote_acquisition = value;
                    }
                }
                "EMBOSS_RS_PREFER_LOCAL_CACHE" => {
                    if let Some(value) = parse_bool(value.as_ref()) {
...
mod tests {
    ...
    fn applies_environment_overrides() {
        let config = ConfigEnvironment::apply_overrides(
            ...
            [
                ("EMBOSS_RS_DATA_DIR", "/tmp/emboss-data"),
                ("EMBOSS_RS_ALLOW_REMOTE", "false"),
            ],
        );
