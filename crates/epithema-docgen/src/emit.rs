...
use epithema_diagnostics::{Diagnostic, ErrorCategory, PlatformError};
...
pub fn emit_generated_docs(
    ...
    fs::write(
        &tool_page,
        normalize_markdown_output(render_tool_page(document, diagnostics)),
    )
    .map_err(|error| {
        PlatformError::new(
            ErrorCategory::Configuration,
...
fn regenerate_index(output_root: &Path) -> Result<(), std::io::Error> {
    ...
    content.push_str(
        "This section contains Markdown pages generated from validated `epithema autodoc` inputs. ",
    );
    content.push_str(
    ...
    fs::write(
        ...
        normalize_markdown_output(content),
    )
}
...
fn render_tool_page(document: &AutodocDocument, diagnostics: &[Diagnostic]) -> String {
    ...
    content.push_str("## Evidence Status\n\n");
    content.push_str(&format!(
        "- Declared evidence baseline: `{}`\n",
        declared_evidence_baseline_label(document)
    ));
    content.push_str(&format!(
        "- Machine-readable validation report: [`../validation/{}.validation.json`](../validation/{}.validation.json)\n",
        document.tool.name, document.tool.name
    ));
    content.push_str(
    ...
    if let Some(validation) = &document.validation {
        content.push_str("## Declared Validation Intent\n\n");
        content.push_str(
            "This section describes what future governed validation should execute or compare. It is not evidence that those runs have already happened.\n\n",
        );
        if validation.required_example_ids.is_empty() {
            content.push_str("- Declared required examples for future validation: none\n");
        } else {
            content.push_str(&format!(
                "- Declared required examples for future validation: `{}`\n",
                validation.required_example_ids.join("`, `")
            ));
        }
        content.push_str(&format!(
            "- Future legacy comparison requested: {}\n",
            yes_no(validation.compare_against_legacy)
        ));
        content.push_str(&format!(
            "- Future execution must capture provenance: {}\n\n",
...
fn declared_evidence_baseline_label(document: &AutodocDocument) -> &'static str {
    match document.provenance.source_mode {
        crate::AutodocSourceMode::RegistryStub => "documented_only",
        crate::AutodocSourceMode::LegacyDerived => "harvested_evidence",
        crate::AutodocSourceMode::Mixed if has_declared_legacy_evidence(document) => {
            "harvested_evidence"
        }
...
fn has_declared_legacy_evidence(document: &AutodocDocument) -> bool {
    !document.provenance.source_references.is_empty()
        || document
            .examples
            .iter()
            .any(|example| example.legacy_reference.is_some())
}
...
fn severity_label(diagnostic: &Diagnostic) -> &'static str {
    match diagnostic.severity {
        epithema_diagnostics::Severity::Error => "error",
        epithema_diagnostics::Severity::Warning => "warning",
        epithema_diagnostics::Severity::Notice => "notice",
    }
}
...
mod tests {
    ...
    fn emits_page_for_minimal_curated_document() {
        ...
        assert!(tool_page.contains("## Evidence Status"));
    ...
    fn emits_page_for_legacy_derived_document_with_notes() {
        ...
        let output_root = std::env::temp_dir().join(format!(
            "epithema-docgen-generated-legacy-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("time should advance")
                .as_nanos()
    ...
    fn emits_stub_page_with_explicit_registry_stub_provenance() {
        ...
        let output_root = std::env::temp_dir().join(format!(
            "epithema-docgen-generated-stub-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("time should advance")
                .as_nanos()
        ...
        assert!(tool_page.contains("- Declared evidence baseline: `documented_only`"));
        assert!(tool_page.contains("- Source mode: `registry-stub`"));
        assert!(tool_page.contains("- Stub generated by: epithema autodoc stub generator"));

        let _ = std::fs::remove_dir_all(&output_root);
    }

    #[test]
    fn normalizes_generated_markdown_to_one_trailing_newline() {
        let document = load_document_from_path(fixture("tests/fixtures/minimal_autodoc.json"))
            .expect("fixture should load");
        let output_root = std::env::temp_dir().join(format!(
            "epithema-docgen-generated-normalized-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("time should advance")
                .as_nanos()
        ...
        let report =
            emit_generated_docs(&document, &[], &output_root).expect("generation should succeed");
        let tool_page = std::fs::read(&report.tool_page).expect("tool page should exist");
        let index_page = std::fs::read(&report.index_page).expect("index page should exist");

        assert!(tool_page.ends_with(b"\n"));
        assert!(!tool_page.ends_with(b"\n\n"));
        assert!(index_page.ends_with(b"\n"));
        assert!(!index_page.ends_with(b"\n\n"));
