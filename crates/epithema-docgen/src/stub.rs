...
pub fn build_stub_document(descriptor: ToolDescriptor) -> AutodocDocument {
    ...
    let mut document = AutodocDocument::new(
        ...
        AutodocProvenance {
            ...
            curated_by: Some("epithema autodoc stub generator".to_owned()),
    ...
    document.sections = vec![
        section(
            ...
            "Overview",
            format!(
                "`{}` is part of the exposed Epithema `{}` cohort. This page is a generated baseline documentation stub produced through the governed autodoc path so the shipped tool surface remains fully documented even where richer harvested narrative or executable examples are still pending.",
...
fn build_needle_document(descriptor: ToolDescriptor) -> AutodocDocument {
    let mut document = AutodocDocument::new(
        ...
        AutodocProvenance {
            ...
            curated_by: Some("epithema maintainers".to_owned()),
            source_references: Vec::new(),
        },
    );
...
fn input_summary(descriptor: ToolDescriptor) -> String {
    match descriptor.family {
        ...
        "feature_tools" => "This tool accepts annotated sequence records through the shared IO layer and operates on simple feature spans preserved in Epithema feature-aware payloads.".to_owned(),
        ...
        _ => "This tool accepts governed Epithema inputs through the shared typed parameter and IO layers. Refer to the implemented CLI help and service definitions for exact parameter requirements.".to_owned(),
    }
}
...
fn output_summary(descriptor: ToolDescriptor) -> String {
    match descriptor.family {
        ...
        _ => "The current implementation emits governed structured results through the shared result/report objects used across Epithema.".to_owned(),
    }
}
