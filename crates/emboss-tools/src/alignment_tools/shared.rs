...
use emboss_io::{parse_aligned_fasta_reader, parse_stockholm_reader};
...
fn map_io_error(error: emboss_io::IoError) -> AlignmentToolError {
...
mod tests {
    ...
    fn detects_stockholm_by_extension() {
        let format = detect_alignment_format(&fixture(
            "../emboss-tools/tests/fixtures/pairwise_alignment.sto",
        ))
    ...
    fn loads_single_alignment_fixture() {
        let alignment = load_alignment(&AlignmentInput::new(fixture(
            "../emboss-tools/tests/fixtures/multiple_alignment.sto",
        )))
