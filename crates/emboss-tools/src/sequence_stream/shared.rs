...
use emboss_io::{parse_embl_reader, parse_fasta_reader, parse_fastq_reader, parse_genbank_reader};
...
fn map_io_error(error: emboss_io::IoError) -> ToolExecutionError {
...
mod tests {
    ...
    fn detects_fasta_by_extension() {
        let format = detect_sequence_format(&fixture(
            "../emboss-tools/tests/fixtures/three_records.fasta",
        ))
