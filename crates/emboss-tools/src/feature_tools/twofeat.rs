...
use emboss_core::{FeatureSelector, SequenceRecord};
...
pub fn twofeat_help() -> &'static str {
    "Usage: emboss-rs twofeat <input> [--a-kind <kind>] [--a-name <name>] [--a-qualifier <key[=value]>] [--a-strand <forward|reverse|unknown>] [--b-kind <kind>] [--b-name <name>] [--b-qualifier <key[=value]>] [--b-strand <forward|reverse|unknown>] [--min-range <count>] [--max-range <count>]\n\nReport neighbouring feature pairs from an annotated EMBL or GenBank input. EMBOSS-RS v1 defines neighbouring pairs conservatively as adjacent features in source order, supports existing selector semantics for feature kind/name/qualifier/strand, and measures optional distances from the nearest feature bounds."
}
...
fn describe_strand_relation(
    a_strand: Option<emboss_core::Strand>,
    b_strand: Option<emboss_core::Strand>,
) -> &'static str {
    match (a_strand, b_strand) {
        (Some(emboss_core::Strand::Forward), Some(emboss_core::Strand::Forward))
        | (Some(emboss_core::Strand::Reverse), Some(emboss_core::Strand::Reverse)) => "same",
        (Some(emboss_core::Strand::Forward), Some(emboss_core::Strand::Reverse))
        | (Some(emboss_core::Strand::Reverse), Some(emboss_core::Strand::Forward)) => "opposite",
        _ => "unknown",
    }
}
...
#[cfg(test)]
mod tests {
    use emboss_core::{FeatureKind, FeatureSelector};
