...
pub struct WaterOutcome {
    ...
    pub result: emboss_core::LocalPairwiseAlignmentResult,
}

/// Returns `water` help text.
#[must_use]
pub fn water_help() -> &'static str {
    "Usage: emboss-rs water <query-input> <target-input> [--gap-open <penalty>] [--gap-extend <penalty>]\n\nPerform deterministic local pairwise alignment between exactly one query record and one target record. EMBOSS-RS v1 uses a Smith-Waterman-style affine-gap alignment core, reports the highest-scoring local region only, and renders the primary payload as Stockholm."
}
