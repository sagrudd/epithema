...
pub fn pasteseq_help() -> &'static str {
    "Usage: emboss-rs pasteseq <asequence> <bsequence> <position>\n\nInsert exactly one sequence record into another after a 1-based position in the main input. Position 0 inserts before the start and position length(asequence) inserts at the end. EMBOSS-RS v1 requires matching molecule kinds and emits one unannotated merged sequence record."
}
