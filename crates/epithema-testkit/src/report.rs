...
use epithema_docgen::{AutodocSourceMode, LegacyReference, load_document_from_path};
use epithema_fixtures::FixtureCatalog;
...
pub fn derive_shipped_cohort_validation_report(
    ...
    Ok(CohortValidationReport {
        ...
        registry_source: "epithema_tools::governed_tool_descriptors".to_owned(),
...
fn derive_method_record(
    repo_root: &Path,
    generated_index: &str,
    descriptor: epithema_tools::ToolDescriptor,
...
mod tests {
  ...
  "diagnostics": []
        ...
        assert_eq!(
            report.source_mode,
            epithema_docgen::AutodocSourceMode::Curated
        );
    ...
    fn shipped_cohort_report_surfaces_visible_gaps() {
        ...
        let psiphi = gap_map.get("psiphi").expect("psiphi should be present");
        assert_eq!(psiphi.evidence_level, CohortEvidenceLevel::ComparedEvidence);
        assert!(psiphi.harvested_legacy_evidence_present);
        ...
        assert!(eprimer3.unresolved_gaps.is_empty());
        ...
        assert_eq!(banana.evidence_level, CohortEvidenceLevel::ComparedEvidence);
        ...
        assert!(banana.unresolved_gaps.is_empty());
        ...
        let syco = gap_map.get("syco").expect("syco should be present");
        assert_eq!(syco.evidence_level, CohortEvidenceLevel::ComparedEvidence);
        assert!(syco.harvested_legacy_evidence_present);
        assert!(syco.executable_validation_present);
        assert!(syco.compared_validation_present);
        assert!(syco.unresolved_gaps.is_empty());
    }
}
