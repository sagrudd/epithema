...
use epithema_docgen::{AutodocDocument, AutodocSourceMode, LegacyReference};
...
mod tests {
    ...
    fn derives_validation_report_from_minimal_autodoc() {
        let document = load_document_from_path(fixture(
            "../epithema-docgen/tests/fixtures/minimal_autodoc.json",
        ))
    ...
    fn derives_harvested_and_pending_comparison_status_from_rich_autodoc() {
        let document = load_document_from_path(fixture(
            "../epithema-docgen/tests/fixtures/rich_autodoc.json",
        ))
        .expect("fixture should load");
    ...
    fn registry_stub_document_projects_as_declared_stub_evidence() {
        let document = epithema_docgen::build_stub_document(
            epithema_tools::governed_tool_descriptors()
                .iter()
                .copied()
        ...
        assert_eq!(
            report.source_mode,
            epithema_docgen::AutodocSourceMode::RegistryStub
        );
        assert_eq!(
    ...
    fn serializes_validation_report_to_json() {
        ...
        let output_path = std::env::temp_dir().join(format!(
            "epithema-testkit-validation-{}.json",
