...
fn render_tool_page(document: &AutodocDocument, diagnostics: &[Diagnostic]) -> String {
    ...
    content.push_str("> Generated from validated autodoc input. Edit the source autodoc document rather than this page.\n\n");
    ...
    if let Some(curated_by) = &document.provenance.curated_by {
        content.push_str(&format!("- Curated by: {}\n", curated_by.trim()));
    }
...
mod tests {
    ...
    use crate::{
        LegacyEmbossSourceRoot, derive_autodoc_from_legacy_root, emit_generated_docs,
