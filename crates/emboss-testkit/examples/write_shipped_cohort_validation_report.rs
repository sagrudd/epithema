//! Writes the shipped cohort validation report as JSON and/or Markdown.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(path) = json_output {
        write_cohort_validation_report_json(&report, path)?;
    } else {
        println!("{}", serde_json::to_string_pretty(&report)?);
    }

    if let Some(path) = markdown_output {
        write_cohort_validation_markdown(&report, path)?;
    } else {
        eprintln!("{}", render_cohort_validation_markdown(&report));
    }
