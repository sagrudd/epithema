...
//! R-exposed methods so that `emboss-r` can compare its public-returned objects
...
impl CrossSurfaceFixtureCatalog {
    ...
    pub fn curated() -> Result<Self, PlatformError> {
        Ok(Self {
            version: 1,
            canonical_owner: "emboss-rs".to_owned(),
...
fn simple_alignment() -> emboss_r_bridge::BridgeAlignmentInput {
    emboss_r_bridge::BridgeAlignmentInput {
        identifier: Some("aln1".to_owned()),
        molecule: Some("dna".to_owned()),
        rows: vec![
            emboss_r_bridge::BridgeAlignmentRowInput {
...
mod tests {
    ...
    fn curated_catalog_covers_curated_method_subset() {
        ...
        let methods = catalog
            ...
            .collect::<Vec<_>>();
        assert_eq!(catalog.canonical_owner, "emboss-rs");
