//! Output helpers for the `epithema` command surface.

use std::path::Path;

use epithema_docgen::{AutodocProcessingSummary, GeneratedDocsReport};
use epithema_io::{write_fasta_string, write_stockholm_string};
use epithema_service::{EpithemaService, InvocationResponse, MethodResult, ResultPayload};
use epithema_testkit::ToolValidationReport;

/// Prints the current governed tool catalogue.
pub fn print_tool_list(service: &EpithemaService) {
    println!("Epithema governed tool catalogue");
...
pub fn print_tool_response(response: &InvocationResponse, service: &EpithemaService) {
...
mod tests {
    ...
    use epithema_docgen::{AutodocProcessingSummary, GeneratedDocsReport};
    use epithema_docgen::{AutodocSourceMode, LegacyReference};
    ...
    fn formats_autodoc_summary() {
        let summary = AutodocProcessingSummary {
            schema_version: "epithema.autodoc/v1".to_owned(),
    ...
    fn renders_sequence_payload_as_fasta() {
        ...
        let sequence = epithema_core::SequenceRecord::new(
            epithema_core::SequenceIdentifier::new("seq1").expect("identifier should build"),
            epithema_core::MoleculeKind::Dna,
            "ACGT",
        )
