//! Command handlers for the `epithema` CLI.
...
use epithema_service::EpithemaService;
...
pub fn run_autodoc_emit_docs(
    ...
    path: &Path,
    docs_output_dir: Option<&Path>,
) -> Result<GeneratedDocsReport, epithema_docgen::AutodocContractError> {
...
pub fn run_autodoc_emit_validation_stub(
    path: &Path,
    validation_output_path: Option<&Path>,
) -> Result<ToolValidationReport, epithema_docgen::AutodocContractError> {
    let document = load_document_from_path(path)?;
    let report =
        derive_validation_report(&document).map_err(epithema_docgen::AutodocContractError::from)?;
    let output_path = validation_output_path
        .map(Path::to_path_buf)
        .unwrap_or_else(|| default_validation_output_path(&report.tool_name));
    write_validation_report_json(&report, &output_path)
        .map_err(epithema_docgen::AutodocContractError::from)?;
    Ok(report)
}
...
mod tests {
    ...
    use epithema_service::{EpithemaService, ServiceRegistry};
    ...
    fn service() -> EpithemaService {
        EpithemaService::new(ServiceRegistry::new())
    }
    ...
    fn autodoc_command_loads_valid_fixture() {
        let summary = run_autodoc(
            &service(),
            &fixture("../epithema-docgen/tests/fixtures/minimal_autodoc.json"),
            false,
            None,
            false,
            None,
    ...
    fn autodoc_command_can_emit_generated_docs() {
        let output_root = std::env::temp_dir().join(format!(
            "epithema-cli-autodoc-generated-{}",
    ...
    fn autodoc_command_rejects_provider_artifacts_without_real_provider_implementation() {
        let error = run_autodoc(
            &service(),
            &fixture("../epithema-docgen/tests/fixtures/rich_autodoc.json"),
            false,
            None,
            false,
            None,
