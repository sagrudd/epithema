...
use epithema_core::{NucleotideWobbleError, NucleotideWobbleProfile, nucleotide_wobble_profile};
...
fn map_error(error: NucleotideWobbleError) -> ToolExecutionError {
    let code = match error {
        ...
        NucleotideWobbleError::InvalidCodingSequence(ref cause) => match cause {
            epithema_core::CodonUsageError::NonCodingLength { .. } => {
                "tools.wobble.coding.non_coding_length"
            }
            epithema_core::CodonUsageError::InvalidCodon(_) => "tools.wobble.codon.invalid",
            epithema_core::CodonUsageError::AmbiguousCodon(_) => "tools.wobble.codon.ambiguous",
            epithema_core::CodonUsageError::InternalStopCodon(_) => {
...
mod tests {
    ...
    use epithema_plot_contract::PlotKind;
