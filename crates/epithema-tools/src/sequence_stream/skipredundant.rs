...
pub fn skipredundant_help() -> &'static str {
    "Usage: epithema skipredundant <input>\n\nRemove exact redundant sequences from one local input set. Epithema v1 defines redundancy conservatively as identical normalized sequence content plus molecule kind, keeps the first representative in source order, and does not implement historical alignment-threshold modes."
}
