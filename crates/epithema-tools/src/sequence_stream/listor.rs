...
pub fn listor_help() -> &'static str {
    "Usage: epithema listor <first-input> <second-input> [--operator <OR|AND|XOR|NOT>]\n\nCombine two local sequence sets with a deterministic logical set operator. Epithema v1 compares normalized sequence content plus molecule kind, removes exact duplicates within each input before applying the operator, preserves first-seen representatives, and emits sequence records rather than a historical USA list file."
}
