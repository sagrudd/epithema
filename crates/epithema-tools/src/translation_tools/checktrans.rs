...
use epithema_core::{SequenceRecord, translate_dna_strict};
...
fn compare_pair(
    ...
    let translated = translate_dna_strict(nucleotide.residues()).map_err(|error| {
        let code = match error {
            epithema_core::TranslationError::NonCodingLength { .. } => {
                "tools.checktrans.nucleotide.non_coding_length"
            }
            epithema_core::TranslationError::InvalidCodon(_) => "tools.checktrans.codon.invalid",
            epithema_core::TranslationError::InvalidFrameOffset { .. } => {
                "tools.checktrans.translation.frame_invalid"
            }
            epithema_core::TranslationError::UnsupportedResidue(_) => {
