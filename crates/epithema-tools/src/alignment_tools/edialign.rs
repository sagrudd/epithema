...
pub fn edialign_help() -> &'static str {
    "Usage: epithema edialign <input> [--min-length <length>]\n\nDerive a bounded local multiple alignment by finding the longest exact shared block across two or more sequence records in one input file. Epithema v1 reports only the shared exact block and does not implement the broader historical dynamic-programming surface."
}
...
mod tests {
    ...
    fn fixture(name: &str) -> PathBuf {
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join(format!("../epithema-tools/tests/fixtures/{name}"))
    }
