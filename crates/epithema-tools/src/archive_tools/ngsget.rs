...
pub struct NgsgetParams {
    ...
    /// Whether the invocation cleaned incomplete local artifacts without downloading.
    pub cleanup: bool,
    /// Optional study-level sample subsampling fraction.
...
pub fn ngsget_help() -> &'static str {
    "Usage: epithema ngsget <accession> [--provider auto|ena|sra] [--out <dir>] [--raw] [--threads <n>] [--transport https|auto|aspera] [--ascp <path>] [--aspera-key <path>] [--aspera-rate <rate>] [--check-downloads <path>] [--no-checksum] [--verify-only] [--cleanup] [--subsample <fraction>]\n\nMaterialize generated FASTQ assets for one public ENA or SRA study, sample, experiment, or run accession. Use --raw to include submitted raw/alignment files and provider-native SRA archives. Use --threads to allow 1 to 20 concurrent direct downloads; the default is 1. Use --transport aspera to download ENA-compatible public file URLs with IBM Aspera ascp, or --transport auto to use Aspera when ascp and an auth key are discoverable and otherwise fall back to HTTPS. When --ascp is omitted, ngsget resolves ascp from PATH; --transport aspera fails early if ascp cannot be found. Use --ascp to set the ascp executable, --aspera-key to set the ENA public Aspera auth key, and --aspera-rate to set the ascp target rate. When --aspera-key is omitted, ngsget creates and uses an epithema-managed copy of a discovered Aspera package key; it cannot synthesize a fresh key that ENA would trust. Use --no-checksum to skip MD5 reads during materialization while still recording expected provider MD5 values and checking byte counts when available. Use --verify-only to checksum existing files in --out without downloading. Use --cleanup to remove incomplete local transfer artifacts and rewrite provenance with absent selected assets marked as cleanup_unstarted planned records. Combining --cleanup with --subsample prunes manifest-known eligible local files for samples outside the retained deterministic subset. Use --subsample with a fraction greater than 0 and no more than 1 to deterministically select that fraction of samples from a study query; sample, experiment, and run queries are always acquired completely. Use --check-downloads to recursively search an existing download tree before network retrieval. The service copies verified matches into the output tree, leaves originals intact, resumes partial downloads when possible, trusts epithema verification markers or prior provenance when provider evidence is unchanged, and reports same-name checksum mismatches as failed materialization records."
}
...
pub fn run_ngsget(params: NgsgetParams) -> Result<NgsgetOutcome, ToolExecutionError> {
    Ok(NgsgetOutcome {
        ...
        sample_subsample_fraction: params.sample_subsample_fraction,
    })
}
