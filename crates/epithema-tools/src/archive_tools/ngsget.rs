...
pub fn ngsget_help() -> &'static str {
    "Usage: epithema ngsget <accession> [--provider auto|ena|sra] [--out <dir>] [--raw] [--threads <n>] [--transport https|auto|aspera] [--ascp <path>] [--aspera-key <path>] [--aspera-rate <rate>] [--check-downloads <path>]\n\nMaterialize generated FASTQ assets for one public ENA or SRA study, sample, experiment, or run accession. Use --raw to include submitted raw/alignment files and provider-native SRA archives. Use --threads to allow 1 to 20 concurrent direct downloads; the default is 1. Use --transport aspera to download ENA-compatible public file URLs with IBM Aspera ascp, or --transport auto to use Aspera when ascp and an auth key are discoverable and otherwise fall back to HTTPS. Use --ascp to set the ascp executable, --aspera-key to set the ENA public Aspera auth key, and --aspera-rate to set the ascp target rate. When --aspera-key is omitted, ngsget creates and uses an epithema-managed copy of a discovered Aspera package key; it cannot synthesize a fresh key that ENA would trust. Use --check-downloads to recursively search an existing download tree before network retrieval. The service copies verified matches into the output tree, leaves originals intact, resumes partial downloads when possible, and reports same-name checksum mismatches as failed materialization records."
}
