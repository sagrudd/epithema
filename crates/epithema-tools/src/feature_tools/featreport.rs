...
use epithema_core::{FeatureSelector, summarize_features};
...
fn render_feature_location_from_bounds_and_strand(
    ...
    strand: Option<epithema_core::Strand>,
) -> String {
    match strand {
        Some(epithema_core::Strand::Reverse) => format!("complement({start}..{end})"),
        _ => format!("{start}..{end}"),
    }
}
...
mod tests {
    ...
    fn reports_selected_features_in_stable_order() {
        ...
        let outcome = run_featreport(FeatreportParams {
            ...
            selector: epithema_core::FeatureSelector::Any,
        })
