...
ENA Aspera path. Epithema first resolves `ascp` from `PATH`, then searches next
to the `ascp` executable, the Aspera install prefix, `$CONDA_PREFIX/etc`,
...
epithema ngsget --raw PRJEB53190 \
  ...
  --aspera-key "$HOME/.aspera/sdk/aspera_bypass_dsa.pem"
```
