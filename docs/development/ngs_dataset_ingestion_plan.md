...
ENA Aspera path. Epithema first resolves `ascp` from `PATH`, then searches the
Aspera install prefix, `$CONDA_PREFIX/etc`, `$HOME/.aspera/connect/etc`, and
`$HOME/.aspera/sdk` for supported key names. When a key is found, epithema
copies it to its managed cache with private permissions and uses that cache copy
for later `ngsget` runs. A freshly generated SSH key is not useful because ENA
public FASP endpoints only trust their configured public-data keys.
