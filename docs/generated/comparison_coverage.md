# Family Comparison Coverage Report

This page is generated from the shipped cohort validation report and the governance-alignment report. It exists to make family-by-family compared coverage easy to scan before reprioritizing the next evidence sweep.

## Summary

- Registry source: `emboss_tools::governed_tool_descriptors`
- Governance source: `docs/governance/appendices/family_to_tool_mapping_reference.md`
- Shipped methods: `96`
- Family rows: `10`
- Compared-evidence methods: `84`
- Executable-only methods: `12`
- Harvested-but-not-compared methods: `10`

## Family Coverage Table

| Family | Shipped methods | Compared | Executable-only | Harvested but not compared |
|---|---:|---:|---:|---:|
| Core Retain — Sequence editing and manipulation | 23 | 14 | 9 | 9 |
| Core Retain — Core sequence statistics and composition | 16 | 15 | 1 | 1 |
| Core Retain — Basic sequence IO and conversion | 18 | 17 | 1 | 0 |
| Core Retain — Simple motif, pattern, and regular-expression search | 12 | 11 | 1 | 0 |
| Core Retain — Alignment read-write and post-processing | 13 | 13 | 0 | 0 |
| Core Retain — ORF and translation-adjacent utilities | 4 | 4 | 0 | 0 |
| Modernize — Rework — Plotting and visualization tools | 2 | 2 | 0 | 0 |
| Modernize — Rework — Protein property and structural-summary utilities | 2 | 2 | 0 | 0 |
| Modernize — Rework — Remote retrieval and archive acquisition | 4 | 4 | 0 | 0 |
| Modernize — Rework — Restriction-enzyme design and analysis | 2 | 2 | 0 | 0 |
