...
Epithema supports Aspera acceleration through conda-compatible user environments. Install `ascli` with `micromamba create -n aspera -c conda-forge -c bioconda aspera-cli`, activate the environment, and run `ascli config transferd install` to install IBM's Aspera transfer runtime under `$HOME/.aspera/sdk`. Add `$HOME/.aspera/sdk` to the environment `PATH` through `$CONDA_PREFIX/etc/conda/activate.d/aspera-sdk.sh`, then verify `command -v ascp` and `ascli config ascp show`. The Bioconda `aspera-cli` package alone does not place `ascp` on `PATH`; the transfer runtime install is required. Epithema searches `$CONDA_PREFIX/etc`, `$HOME/.aspera/connect/etc`, and `$HOME/.aspera/sdk` for `asperaweb_id_dsa.openssh`, `aspera_tokenauth_id_dsa`, `aspera_tokenauth_id_rsa`, `aspera_bypass_dsa.pem`, or `aspera_bypass_rsa.pem`. The `aspera_bypass_*.pem` files are SDK bypass keys created by the Aspera transfer runtime and are accepted by epithema's ENA Aspera path. If discovery fails, pass `--ascp $HOME/.aspera/sdk/ascp --aspera-key $HOME/.aspera/sdk/aspera_bypass_dsa.pem` explicitly.
