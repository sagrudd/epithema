...
`cai` is part of the exposed EMBOSS-RS `codon_tools` cohort. This page is a generated baseline documentation stub produced through the governed autodoc path so the shipped tool surface remains fully documented even where richer harvested narrative or executable examples are still pending.
...
This method is implemented and exposed through `emboss-rs cai`. The generated tool page and the machine-readable validation stub at [`../validation/cai.validation.json`](../validation/cai.validation.json) are current. No richer autodoc examples are declared in this contract yet; future prompts should replace or extend this stub with harvested or executable evidence rather than hand-maintaining the generated page directly.
