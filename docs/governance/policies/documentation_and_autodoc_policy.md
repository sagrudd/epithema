# Epithema Documentation and Auto-Documentation Policy

**Status:** Formal development policy addendum  
**Applies to:** `epithema`, `epithemaR`, project documentation, release process, and GitHub Pages publication  
**Date:** 2026-04-15
...
This policy establishes the mandatory documentation, auto-documentation, and publication standards for the Epithema project.

The purpose of this policy is to ensure that Epithema maintains credible, reproducible, and continuously refreshed documentation grounded in historical EMBOSS artefacts and executable evidence from the modern codebase.
...
- all user-facing `epithema` tools
...
- the supporting relationship between historical EMBOSS artefacts and Epithema documentation generation
...
`epithema` **must** expose an `autodoc` subcommand.
...
`epithema autodoc` **shall** consume a JSON document describing:
...
- any mappings from historical EMBOSS examples to Epithema invocations
...
It is the responsibility of `epithema autodoc` to derive documentation JSON from the original EMBOSS GitHub artefacts available at:
...
- construction of EMBOSS-to-Epithema example mappings
- generation of normalized documentation JSON suitable for downstream Sphinx rendering

Where historical artefacts are incomplete, inconsistent, or ambiguous, `epithema autodoc` **must** record that fact explicitly in generated metadata or diagnostics.

### 3.5 Data provenance and acquisition discipline

Data used in documentation **shall** be downloaded for reporting purposes only by methods exposed elsewhere within the `epithema` software.

This means:

- `autodoc` must not introduce an undocumented shadow retrieval path
- all downloads used for documentation must flow through supported `epithema` retrieval or acquisition methods
...
`epithema autodoc` **shall** be run each time a new tool is introduced into the software.
...
- public project information relevant to the Epithema reboot
...
Documentation generation is not an auxiliary convenience. It is a core subsystem of Epithema and must be reflected in architecture, build orchestration, testing, and release policy.

### 4.2 Auto-documentation must be reproducible

`epithema autodoc` must operate reproducibly from declared inputs, declared provenance, and versioned source artefacts.
...
Any data acquisition performed for documentation must reuse product methods. Documentation must not depend on capabilities that are absent from the actual Epithema user surface.
...
- Epithema invocation definition
- input artefact list
...
- `epithema autodoc` runs should be automatable in CI or scheduled release workflows
...
2. No documentation download path may bypass existing Epithema retrieval methods.
...
- a dedicated `autodoc` implementation in `epithema`
...
The Epithema project shall maintain product documentation using Sphinx with the Read the Docs template. The project shall provide a canonical `epithema autodoc` command that consumes documentation JSON describing input artefact provenance, method execution context, and documentation text. The `autodoc` capability shall derive documentation JSON from historical artefacts in the public EMBOSS repository where possible, and any data downloaded for documentation purposes shall be obtained only through methods already exposed elsewhere in Epithema. Auto-documentation shall be run whenever a new tool is introduced, and the project shall maintain formal GitHub Pages from the start of development as an official public documentation surface.
