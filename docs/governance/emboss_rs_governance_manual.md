...
`emboss-rs` is a full-platform reboot of the historic EMBOSS collection of
...
- the Rust and CLI surface delivered through `emboss-rs`
- the R surface delivered through the sister repository `emboss-r`

`emboss-r` is a peer surface, not merely a plotting adapter. It is responsible
for R-facing access to the EMBOSS-RS method catalog and is the exclusive
...
Plotting is governed as an R responsibility. `emboss-rs` may emit plot-ready
data, but graphical rendering belongs in `emboss-r`.

## 4. Scope and Tool-Family Policy

`emboss-rs` is governed as a coherent family of related EMBOSS-compatible or
...
- R-facing plotting workflows remain in `emboss-r`
...
for `emboss-rs`, and governance changes must leave the repository ready for a
future Sphinx-based documentation build rather than forcing a later structural
migration.

The governing rules are:

- `emboss-rs` must expose `emboss-rs autodoc`
...
traceability, auditability, and sponsor-visible progress during the EMBOSS-RS
reboot.
...
`emboss-rs` must maintain disciplined repository structure and module naming as
...
- the `emboss-rs <tool> ...` surface remains consistent with governance
...
project-specific terms, including `emboss-rs`, `emboss-r`, `autodoc`, and the
